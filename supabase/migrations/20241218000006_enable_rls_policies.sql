...
-- セッションのuser_idはJWTのclaimsから取得する想定
...
CREATE OR REPLACE FUNCTION auth.is_section_participant(section_uuid UUID)
RETURNS BOOLEAN AS $$
BEGIN
  RETURN EXISTS (
...
CREATE OR REPLACE FUNCTION auth.is_section_creator(section_uuid UUID)
RETURNS BOOLEAN AS $$
BEGIN
  RETURN EXISTS (
...
CREATE POLICY users_select_policy ON users
  FOR SELECT
  USING (auth.current_user_id() IS NOT NULL AND deleted_at IS NULL);
...
CREATE POLICY users_update_policy ON users
  FOR UPDATE
  USING (auth.is_admin() OR id = auth.current_user_id())
  WITH CHECK (auth.is_admin() OR id = auth.current_user_id());
...
CREATE POLICY sections_update_policy ON sections
  FOR UPDATE
  USING (auth.is_section_creator(id) OR auth.is_admin())
  WITH CHECK (auth.is_section_creator(id) OR auth.is_admin());
...
CREATE POLICY section_participants_insert_policy ON section_participants
  FOR INSERT
  WITH CHECK (auth.is_section_creator(section_id) OR auth.is_admin());

-- DELETE: セクション作成者または管理者
CREATE POLICY section_participants_delete_policy ON section_participants
  FOR DELETE
  USING (auth.is_section_creator(section_id) OR auth.is_admin());
...
CREATE POLICY games_insert_policy ON games
  FOR INSERT
  WITH CHECK (auth.is_section_participant(section_id) OR auth.is_admin());

-- UPDATE: セクション参加者または管理者
CREATE POLICY games_update_policy ON games
  FOR UPDATE
  USING (auth.is_section_participant(section_id) OR auth.is_admin())
  WITH CHECK (auth.is_section_participant(section_id) OR auth.is_admin());
